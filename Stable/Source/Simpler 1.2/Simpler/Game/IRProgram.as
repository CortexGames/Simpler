package Simpler.Game
{
	...
	public class IRProgram extends IRScene
	{
		...
		private var m_aStates:Vector.<String> = new Vector.<String>(); // Store Game States
		...
		private var UpdateState:Function = function():void{}; // Stores the Update State
		
		protected var m_cState:IRObject = null; // Store the Game State
		...
		private function onAdded(e:starling.events.Event):void
		{
			...
			stage.addEventListener(starling.events.Event.ENTER_FRAME, IRUpdate); // Add Enter Frame Event Listener
			...
			m_tTimer.addEventListener(TimerEvent.TIMER, FPSLoop); // Set an Update Timer
		...
		private function IRUpdate(e:starling.events.Event):void
		{
			Global.DELTATIME = (getTimer() - m_uDeltaLast) * 0.001; // Set Delta Time
			m_uDeltaLast = getTimer(); // Store Time
			if(Global.PAUSE)
			{
				if(Global.PAUSESTATE != null)
				{
					Global.PAUSESTATE.Update();
				}
			} // Run Pause State
			else
			{
			...
			UpdateDeletion(); // Update Deletion
		...
		private function UpdateDeletion():void
		{
		...
		private function SwitchState():void
		{
			if(Global.GAMESTATE != Global.NEWSTATE)
			{
				...
				if(m_cState != null)
				{
					m_cState.Destroy(); // Destroy the State
					m_cState = null; // Format the State
				}
				...
				for(i = 0;i < m_aStates.length;i++)
				{
					if(m_aStates[i] == Global.NEWSTATE)
					{
						...
						UpdateState = this["State" + m_aStates[i]]; // Store State Update Function
						Ran = true; // Function has run
					}
				}
				if(!Ran)
				{
					trace("[Simpler] Error 1001: " + Global.NEWSTATE + " has no state function, check spellings!");
		...
		public function Config(collision_:IRCollisionBase = null, assets_:Class = null, physics_:* = null):void
		{
				Global.COLLISION = collision_; // Set the Collision
				Global.ASSETS = assets_["getInstance"]; // Set the AssetLoader
				Global.ASSETS(); // Initialize the Asset Loader
		...
		private function FPSLoop(e:TimerEvent):void
		{
			...
			if(m_uFPSTimer - 1000 > m_uFPSLast)
			{
				...
				if(m_iFPSTotal == 1)
				{
					trace("[Simpler] Error 1001: The FPS is to low!");
				}
			}
			m_iFPSCount++;
		} // Track the FPS
		...
		private function FPSProtection():void
		{
			...
			{
				trace("[Simpler] Error 1002: Launching new state " + m_aStates[0] + " due to low FPS!");
